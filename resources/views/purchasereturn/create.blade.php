                            ...
                            <div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="purchase_no" class="form-control"
                                        placeholder="Purchase Number" value="{{ $purchase->purchase_no }}">
                                </div>
                            </div>
                        </div>
                        <hr>
...
$("body").on("click", ".add-return", function() {
    ...
    var mrp = $(this).parents("tr").attr('data-mrp');
    var tp = $(this).parents("tr").find("td").eq(3).text();
    var qty = $(this).parents("tr").find("td").eq(2).text();
    var amount = parseFloat(qty * tp).toFixed(2);
    console.log(tp);
    $(".return-table tbody").append('<tr class="i" data-item_id=' + item_id + ' data-mrp=' + mrp + '>' +
        '<td>' + itemname + '</td>' +
    ...
    if (return_type == "1") {
        ...
        $.ajax({
            ...
            success: function(data) {
                console.log(data);
                url = "{{ url('Purchase-Return/show')}}" + '/' + data,
                    window.location = url;
            },
            error: function(data) {
